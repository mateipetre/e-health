// eslint-disable-next-line import/no-anonymous-default-export
export default {
  patients: {
    label: 'Пациенты',
    warning: 'Внимание!',
    viewPatients: 'Просмотр пациентов',
    noPatients: 'Пациентов еще нет, давайте добавим первого!',
    viewPatient: 'Просмотр пациента',
    createPatient: 'Создать пациента',
    editPatient: 'Редактировать пациента',
    newPatient: 'Новый пациент',
    updatePatient: 'Изменить данные пациента',
    patientsList: 'Список пациентов',
    successfullyCreated: 'Пациент успешно добавлен',
    successfullyAddedNote: 'Новая заметка успешно добавлена',
    successfullyAddedRelatedPerson: 'Успешно добавлен новый родственник',
    successfullyUpdated: 'Успешно обновлена информация пациента ',
    possibleDuplicatePatient: 'Возможный дубликат пациента:',
    duplicatePatientWarning:
      'Пациент с такими данными уже присутствует в базе. Вы уверен, что хотите создать нового пациента?',
  },
}
