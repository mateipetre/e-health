...
const NewNote = () => {
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      const response = await axios.post(`/doctors/66cbb31025bfefa0333b2882/notes`, newNote);
     if (response.status === 201) {
        setSuccessOpen(true);
      }
    } catch (error) {
      console.error('Error adding new note:', error);
      setErrorMessage(error.response?.data?.message || 'Server error while adding note');
    ...
    <Box sx={{ padding: '20px', boxSizing: 'border-box', maxWidth: '2100px' }}>
      ...
      <HICSuccessModal
        ...
        actions={[
          {
            label: 'View Notes',
            onClick: () => navigate('/doctor/notes/view'),
            variant: 'contained',
            color: 'primary',
          },
        ]}
