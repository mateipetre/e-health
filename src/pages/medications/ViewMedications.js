...
import ViewMedicationModal from './view/ViewMedicationModal';
    ...
    const fetchMedications = async () => {
      try {
        ...
        const formattedMedications = response.data.map((med) => ({
          ...med,
          creationDate: format(new Date(med.creationDate), 'yyyy-MM-dd'),
          updateDate: format(new Date(med.updateDate), 'yyyy-MM-dd'),
        }));
                ...
                data={filteredData.map((row, index) => ({
                  ...
                  actions: (
                    <Box sx={{ display: 'flex', gap: '16px' }}>
                      <Tooltip title="View medication details" arrow>
                        <img
                          src="/view.png"
                          alt="View"
                          style={{ cursor: 'pointer', width: '32px' }}
                          onClick={() => openModal(row)} // Open modal with selected medication
                        />
                      </Tooltip>
                    </Box>
                  ),
