...
const DeleteAllergy = () => {
  ...
  useEffect(() => {
    const fetchAllergies = async () => {
      try {
        const response = await axios.get(`http://localhost:5000/patients/${patientId}/allergies`);
  ...
  const handleConfirmDelete = async () => {
    if (selectedRow) {
      try {
        await axios.delete(`http://localhost:5000/patients/${patientId}/allergies/${selectedRow._id}`);
